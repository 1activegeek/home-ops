...
- **Authentik URL**: `https://auth.literal:${SECRET_DOMAIN}` (external) or `http://authentik.security.svc.cluster.local:9000` (internal)
- **Domain**: `literal:${SECRET_DOMAIN}`
...
- **App namespace pattern**: app hostnames follow `<release-name>.literal:${SECRET_DOMAIN}`
...
1. Log into Authentik at `https://auth.literal:${SECRET_DOMAIN}`
...
AUTH_FLOW=$(curl -s -H "Authorization: Bearer ${AUTHENTIK_TOKEN}" \
  "https://auth.literal:${SECRET_DOMAIN}/api/v3/flows/instances/?designation=authorization&slug=default-provider-authorization-implicit-consent" \
...
INVAL_FLOW=$(curl -s -H "Authorization: Bearer ${AUTHENTIK_TOKEN}" \
  "https://auth.literal:${SECRET_DOMAIN}/api/v3/flows/instances/?designation=invalidation" \
...
PROVIDER_RESPONSE=$(curl -s -X POST \
  ...
  "https://auth.literal:${SECRET_DOMAIN}/api/v3/providers/oauth2/" \
  -d "{
...
APP_RESPONSE=$(curl -s -X POST \
  -H "Authorization: Bearer ${AUTHENTIK_TOKEN}" \
  -H "Content-Type: application/json" \
  "https://auth.literal:${SECRET_DOMAIN}/api/v3/core/applications/" \
  -d "{
    \"name\": \"${APP_NAME}\",
    \"slug\": \"${APP_NAME}\",
...
curl -s -X PATCH \
  -H "Authorization: Bearer ${AUTHENTIK_TOKEN}" \
  -H "Content-Type: application/json" \
  "https://auth.literal:${SECRET_DOMAIN}/api/v3/core/applications/${APP_SLUG}/" \
...
security:
  oidc:
    issuer-url: https://auth.literal:${SECRET_DOMAIN}/application/o/gatus/
    redirect-url: https://gatus.literal:${SECRET_DOMAIN}/authorization-code/callback
...
grafana.ini:
  auth.generic_oauth:
    ...
    scopes: openid email profile
    auth_url: https://auth.literal:${SECRET_DOMAIN}/application/o/authorize/
    token_url: https://auth.literal:${SECRET_DOMAIN}/application/o/token/
    api_url: https://auth.literal:${SECRET_DOMAIN}/application/o/userinfo/
...
OAuth2 Provider:
  ...
  Discovery URL: https://auth.literal:${SECRET_DOMAIN}/application/o/<APP_NAME>/.well-known/openid-configuration
